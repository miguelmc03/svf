...
class SaleSimpleOrder(models.Model):
    _name = "sale.simple.order"
    ...
    def _get_invoice_status(self):
        ...
        line_invoice_status_all = [(d['order_id'][0], d['invoice_status']) for d in self.env['sale.simple.order.line'].read_group([('order_id', 'in', self.ids), ('product_id', '!=', deposit_product_id.id)], ['order_id', 'invoice_status'], ['order_id', 'invoice_status'], lazy=False)]
        for order in self:
    ...
    def get_empty_list_help(self, help):
        self = self.with_context(
            empty_list_help_document_name=_("quotations"),
        )
        return super(SaleSimpleOrder, self).get_empty_list_help(help)
    ...
    order_line = fields.One2many('sale.simple.order.line', 'order_id', string='Order Lines', states={'cancel': [('readonly', True)], 'done': [('readonly', True)]}, copy=True, auto_join=True)
    ...
    transaction_ids = fields.Many2many('payment.transaction', 'sale_order_simple_transaction_rel', 'sale_order_id', 'transaction_id',
                                       string='Transactions', copy=False, readonly=True)
    authorized_transaction_ids = fields.Many2many('payment.transaction', compute='_compute_authorized_transaction_ids',
                                                  string='Authorized Transactions', copy=False, readonly=True)
    tag_ids = fields.Many2many('crm.lead.tag', 'sale_simple_order_tag_rel', 'order_id', 'tag_id', string='Tags')
    opportunity_id = fields.Many2one(
        'crm.lead', string='Opportunity', check_company=True,
        domain="[('type', '=', 'opportunity'), '|', ('company_id', '=', False), ('company_id', '=', company_id)]")
    partner_name = fields.Char(related='opportunity_id.partner_name')
    email = fields.Char(related='opportunity_id.email_from')
    street = fields.Char(related='opportunity_id.street')
            ...
            for cell in dirty_cells:
                ...
                if diff and order_lines:
                    ...
                    if not default_so_line_vals:
                        OrderLine = self.env['sale.simple.order.line']
    ...
    def _compute_access_url(self):
        super(SaleSimpleOrder, self)._compute_access_url()
        for order in self:
    ...
    def unlink(self):
        ...
        return super(SaleSimpleOrder, self).unlink()
    ...
    def _track_subtype(self, init_values):
        ...
        return super(SaleSimpleOrder, self)._track_subtype(init_values)
    ...
    def onchange_partner_id(self):
        ...
        if self.env['ir.config_parameter'].sudo().get_param('account.use_invoice_terms') and self.env.company.invoice_terms:
            values['note'] = self.with_context().env.company.invoice_terms
    ...
    def create(self, vals):
        if vals.get('name', _('New')) == _('New'):
            ...
            if 'company_id' in vals:
                vals['name'] = self.env['ir.sequence'].with_context(force_company=vals['company_id']).next_by_code(
                    'sale.simple.order', sequence_date=seq_date) or _('New')
            else:
                vals['name'] = self.env['ir.sequence'].next_by_code('sale.simple.order', sequence_date=seq_date) or _('New')
        ...
        result = super(SaleSimpleOrder, self).create(vals)
        return result
    ...
    def _write(self, values):
        ...
        if self.env.context.get('mail_activity_automation_skip'):
            return super(SaleSimpleOrder, self)._write(values)
    ...
    def _name_search(self, name, args=None, operator='ilike', limit=100, name_get_uid=None):
        ...
        return super(SaleSimpleOrder, self)._name_search(name, args=args, operator=operator, limit=limit, name_get_uid=name_get_uid)
    ...
    def action_quotation_send(self):
        ...
        template = self.env.ref('3mit_changes_crm.email_template_slip', False)
        for slip in self:
    ...
    def message_post(self, **kwargs):
        ...
        return super(SaleSimpleOrder, self.with_context(mail_post_autofollow=True)).message_post(**kwargs)
    ...
    def _amount_by_group(self):
        for order in self:
            ...
            fmt = partial(formatLang, self.with_context().env, currency_obj=currency)
            res = {}
            for line in order.order_line:
    ...
    def _notify_get_groups(self):
        ...
        groups = super(SaleSimpleOrder, self)._notify_get_groups()
    ...
    def _get_share_url(self, redirect=False, signup_partner=False, pid=None):
        ...
        return super(SaleSimpleOrder, self)._get_share_url(redirect, signup_partner, pid)
...
class SaleSimpleOrderLine(models.Model):
    _name = 'sale.simple.order.line'
    ...
    def write(self, values):
        ...
        result = super(SaleSimpleOrderLine, self).write(values)
        return result

    order_id = fields.Many2one('sale.simple.order', string='Order Reference', required=True, ondelete='cascade', index=True, copy=False)
    name = fields.Text(string='Description', required=True)
    sequence = fields.Integer(string='Sequence', default=10)

    invoice_lines = fields.Many2many('account.move.line', 'sale_simple_order_line_invoice_rel', 'order_line_id', 'invoice_line_id', string='Invoice Lines', copy=False)
    invoice_status = fields.Selection([
        ('upselling', 'Upselling Opportunity'),
        ('invoiced', 'Fully Invoiced'),
        ('to invoice', 'To Invoice'),
    ...
    def _name_search(self, name, args=None, operator='ilike', limit=100, name_get_uid=None):
        ...
        return super(SaleSimpleOrderLine, self)._name_search(name, args=args, operator=operator, limit=limit, name_get_uid=name_get_uid)
    ...
    def _check_line_unlink(self):
        ...
        :rtype: recordset sale.simple.order.line
    ...
    def unlink(self):
        ...
        return super(SaleSimpleOrderLine, self).unlink()
    ...
    def _get_sale_order_line_multiline_description_variants(self):
        ...
        for ptav in (no_variant_ptavs - custom_ptavs):
            name += "\n" + ptav.with_context().display_name

        # display the is_custom values
        for pacv in self.product_custom_attribute_value_ids:
            name += "\n" + pacv.with_context().display_name
